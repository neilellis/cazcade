...
public class FountainPoolDAOImpl implements FountainPoolDAO {
    ...
    public LSDTransferEntity updatePoolObjectNoTx(@Nonnull final LiquidSessionIdentifier identity, @Nonnull final LiquidSessionIdentifier editor, @Nonnull final LSDTransferEntity entity, @Nullable final LSDPersistedEntity pool, @Nonnull final LSDPersistedEntity origPersistedEntity, final boolean internal, final LiquidRequestDetailLevel detail) throws InterruptedException {
        ...
        if (!entity.getURI().toString().toLowerCase().equals(origPersistedEntity.getAttribute(LSDAttribute.URI))) {
            throw new CannotChangeURIException("Tried to change the URI of %s to %s", origPersistedEntity.getAttribute(LSDAttribute.URI), entity.getURI().toString());
        }
...
//            recalculateCentreImage(parent, poolObject);
    ...
    LSDTransferEntity deletePoolObjectNoTx(final boolean internal, final LiquidRequestDetailLevel detail, @Nonnull final Transaction transaction, @Nonnull final LSDPersistedEntity persistedEntity) throws Exception {
        if (persistedEntity.isDeleted()) {
            throw new DeletedEntityException("The entity %s is already deleted so cannot be deleted again.", persistedEntity.hasAttribute(LSDAttribute.URI) ? persistedEntity.getAttribute(LSDAttribute.URI) : "<unknown-uri>");
        }
        ...
        if (relationship == null) {
            throw new OrphanedEntityException("The entity %s is orphaned so cannot be deleted.", persistedEntity.hasAttribute(LSDAttribute.URI) ? persistedEntity.getAttribute(LSDAttribute.URI) : "<unknown-uri>");
        }
//        recalculateCentreImage(relationship.getOtherNode(persistedEntityImpl), persistedEntityImpl);
    ...
    @Override
    public void visitNodeNoTx(@Nonnull final LSDPersistedEntity persistedEntityImpl, @Nonnull final LiquidSessionIdentifier identity) throws InterruptedException {
        fountainNeo.begin();
        try {
            ...
            final LSDPersistedEntity pool = convertToPoolFromPoolOrObject(persistedEntityImpl);
            ...
            indexDAO.incrementBoardActivity(persistedEntityImpl);
            indexDAO.visitBoard(persistedEntityImpl, identity.getAliasURL());
        } finally {
            fountainNeo.end();
        }
    }
    ...
    public LSDPersistedEntity movePoolObjectNoTx(@Nonnull final LiquidURI object, @Nullable final Double x, @Nullable final Double y, @Nullable final Double z) throws Exception {
        ...
        try {
            ...
            if (viewPersistedEntity == null) {
                throw new EntityNotFoundException("No view relationship for %s(%s)", persistedEntity.getAttribute(LSDAttribute.URI), object);
            }
            if (x != null) {
            ...
            if (parentRel == null) {
                throw new OrphanedEntityException("The entity %s (%s) is orphaned so cannot be moved.", object.toString(), persistedEntity.hasAttribute(LSDAttribute.URI) ? persistedEntity.getAttribute(LSDAttribute.URI) : "<unknown-uri>");
            }
//            recalculateCentreImage(parentRel.getOtherNode(persistedEntityImpl), persistedEntityImpl);
    ...
    @Nullable
    public LSDTransferEntity convertNodeToEntityWithRelatedEntitiesNoTX(@Nonnull final LiquidSessionIdentifier identity, @Nullable final LSDPersistedEntity persistedEntity, @Nullable final LSDPersistedEntity parent, final LiquidRequestDetailLevel detail, final boolean internal, final boolean historical) throws InterruptedException {
        fountainNeo.begin();
        try {
            if (persistedEntity == null) {
                return null;
            ...
            for (final FountainRelationship relationship : iterable) {
                ...
                entity.addSubEntity(LSDAttribute.VIEW, relationship.getOtherNode(persistedEntity).convertNodeToLSD(detail, internal), true);
                done = true;
            }
    ...
    @Nonnull
    public LSDPersistedEntity addCommentNoTX(@Nullable final LSDPersistedEntity targetPersistedEntity, @Nonnull final LSDTransferEntity entity, @Nullable final LiquidURI author) throws InterruptedException {
        fountainNeo.begin();
        try {
            final LSDTransferEntity entityCopy = entity.copy();
            if (author == null) {
            ...
            final String uri = targetPersistedEntity.getAttribute(LSDAttribute.URI) + "~" + "comment-" + author.getSubURI().getSubURI() + System.currentTimeMillis();
            commentPersistedEntity.setAttribute(LSDAttribute.URI, uri);

            if (targetPersistedEntity.hasRelationship(COMMENT, Direction.OUTGOING)) {
                final Iterable<FountainRelationship> comments = targetPersistedEntity.getRelationships(COMMENT, Direction.OUTGOING);
            ...
            targetPersistedEntity.createRelationshipTo(commentPersistedEntity, COMMENT);
            ...
            commentPersistedEntity.inheritPermissions(targetPersistedEntity);
            ...
            if (targetPersistedEntity.hasAttribute(LSDAttribute.COMMENT_COUNT)) {
                commentCount = targetPersistedEntity.getIntegerAttribute(LSDAttribute.COMMENT_COUNT) + 1;
            } else {
                commentCount = getCommentTraverser(targetPersistedEntity, FountainNeoImpl.MAX_COMMENTS_DEFAULT).getAllNodes().size();
            }
            log.debug("Comment count is now {0}", commentCount);
            targetPersistedEntity.setAttribute(LSDAttribute.COMMENT_COUNT, String.valueOf(commentCount));
            indexDAO.syncCommentCount(targetPersistedEntity);
            indexDAO.incrementBoardActivity(targetPersistedEntity);
    ...
    @Nullable
    public Collection<LSDBaseEntity> getCommentsTx(@Nonnull final LiquidSessionIdentifier identity, @Nonnull final LiquidURI uri, final int max, final boolean internal, final LiquidRequestDetailLevel detail) throws InterruptedException {
        fountainNeo.begin();
        try {
            final List<LSDBaseEntity> comments = new ArrayList<LSDBaseEntity>();
            final Transaction transaction = fountainNeo.beginTx();
            try {
                final LSDPersistedEntity persistedEntity = fountainNeo.findByURI(uri);
                if (persistedEntity == null) {
