...
public class FountainTestDataBuilderClient {
    ...
    public static void main(final String[] args) throws IOException {
        ...
        client.getState().setCredentials(new AuthScope(FountainTestClientSupport.host, FountainTestClientSupport.port,
                                                       AuthScope.ANY_REALM
        ), new UsernamePasswordCredentials("anon", "anon")
                                        );
        ...
        final LSDBaseEntity sessionEntity = FountainTestClientSupport.callRESTApiWithGet(new ClientSession(client, null, username),
...
//        assertTrue(aliasEntity.getTypeDef().getPrimaryType().toString().equals(LSDTypeDictionary.ALIAS.getAttribute()));
    ...
    private static void initHomePool() throws IOException {
        final LSDBaseEntity poolEntity = FountainTestClientSupport.callRESTApiWithGet(clientSession,
                                                                                      "pool.xml?url=pool:///people/" + username
                                                                                     );
        final String entityId = poolEntity.getUUID().toString();
        FountainTestClientSupport.putEntityToURL(clientSession, "pool/" + entityId + ".xml",
                                                 FountainTestClientSupport.createEntityFromPropertyfile("TestObject1"),
                                                 "alias:cazcade:" + username
                                                );
        FountainTestClientSupport.putEntityToURL(clientSession, "pool/" + entityId + ".xml",
                                                 FountainTestClientSupport.createEntityFromPropertyfile("TestObject2"),
                                                 "alias::cazcade:" + username
                                                );
        FountainTestClientSupport.putEntityToURL(clientSession, "pool/" + entityId + ".xml",
                                                 FountainTestClientSupport.createEntityFromPropertyfile("TestObject3"),
                                                 "alias:cazcade:" + username
                                                );
        FountainTestClientSupport.putEntityToURL(clientSession, "pool/" + entityId + ".xml",
                                                 FountainTestClientSupport.createEntityFromPropertyfile("TestObject4"),
                                                 "alias:cazcade:" + username
                                                );
        FountainTestClientSupport.putEntityToURL(clientSession, "pool/" + entityId + ".xml",
                                                 FountainTestClientSupport.createEntityFromPropertyfile("TestObject5"),
                                                 "alias:youtube:HomeAdditionPlus"
                                                );
        final LSDBaseEntity testEntity = FountainTestClientSupport.callRESTApiWithGet(clientSession,
                                                                                      "pool.xml?url=pool:///people/" +
                                                                                      username +
                                                                                      "%23TestObject4"
                                                                                     );
        System.err.println(testEntity.toString());
        assertTrue(testEntity.getTypeDef().getPrimaryType().getClassOnlyType().asString().equals(
...
//        FountainTestClientSupport.postEntityToURL(clientSession, "pool/" + streamId + ".xml", FountainTestClientSupport.createEntityFromPropertyfile("TwitterFeed"), "alias:cazcade:"+username);
        FountainTestClientSupport.putEntityToURL(clientSession, "pool/" + streamId + ".xml",
                                                 FountainTestClientSupport.createEntityFromPropertyfile("RSSFeed1"),
                                                 "alias:cazcade:" + username
                                                );
        FountainTestClientSupport.putEntityToURL(clientSession, "pool/" + streamId + ".xml",
                                                 FountainTestClientSupport.createEntityFromPropertyfile("RSSFeed2"),
                                                 "alias:cazcade:" + username
                                                );
        FountainTestClientSupport.putEntityToURL(clientSession, "pool/" + streamId + ".xml",
                                                 FountainTestClientSupport.createEntityFromPropertyfile("RSSFeed3"),
                                                 "alias:cazcade:" + username
                                                );
    }
