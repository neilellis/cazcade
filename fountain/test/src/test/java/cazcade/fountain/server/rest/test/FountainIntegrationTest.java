...
public class FountainIntegrationTest {
    ...
    private void initaliseTestUser() throws IOException {
        ...
        final LSDBaseEntity aliasEntity = FountainTestClientSupport.callRESTApiWithGet(userSession, "alias.xml?uri=alias:twitter:"
                                                                                                    + testUsername);
    ...
    public void test() throws IOException, InterruptedException {
        ...
        Assert.assertTrue(testEntity.getTypeDef()
                                    .getPrimaryType()
                                    .getClassOnlyType()
                                    .asString()
    ...
    public void testDeletePool() throws InterruptedException, IOException {
        ...
        final LSDBaseEntity poolEntity = FountainTestClientSupport.callRESTApiWithGet(userSession, "pool.xml?uri=" +
                                                                                                   LiquidURI +
                                                                                                   "/testchildfordelete");
        final LSDBaseEntity deletionEntity = FountainTestClientSupport.callRESTApiWithGet(userSession, "pool/delete?uri="
                                                                                                       + poolEntity.getURI());
        Thread.sleep(500);
        try {
    ...
    public void testOtherPublicPool() throws IOException, InterruptedException {
        ...
        final LSDBaseEntity entity = FountainTestClientSupport.writeThenGetTestPoolObject(userSession, LiquidURI);
        Thread.sleep(100);
        Assert.assertEquals("Should be able to add to other user's public pool.", "Image.Bitmap.2DBitmap", entity.getTypeDef()
                                                                                                                 .getPrimaryType()
                                                                                                                 .asString());
        listenThread.stop();
    }
    ...
    public void testUpdatePool() throws IOException, InterruptedException {
        final Thread listenThread = FountainTestClientSupport.listenToSession(sessionId, testUserHomePool, credentials);
        final LSDTransferEntity testEntity = FountainTestClientSupport.callRESTApiWithGet(userSession, "pool.xml?uri="
                                                                                                       + testUserHomePool);
        testEntity.setAttribute(LSDAttribute.RIGHTS, CAZCADE_COPYRIGHT_STATEMENT);
        FountainTestClientSupport.putEntityToURL(userSession, "pool/update?uri=" + testUserHomePool, testEntity, "alias:cazcade:"
                                                                                                                 + testUsername);
        Thread.sleep(1000);
        final LSDBaseEntity updatedEntity = FountainTestClientSupport.callRESTApiWithGet(userSession, "pool.xml?uri="
                                                                                                      + testUserHomePool);
        Assert.assertEquals(CAZCADE_COPYRIGHT_STATEMENT, updatedEntity.getAttribute(LSDAttribute.RIGHTS));
        listenThread.stop();
    }
