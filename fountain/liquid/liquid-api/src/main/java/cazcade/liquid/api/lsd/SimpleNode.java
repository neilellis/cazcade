...
public class SimpleNode implements Node {
    ...
    public SimpleNode(final String name, @Nonnull final List list) {
        super();
        this.name = name;
        isArray = list.size() > 1;
        for (final Object o : list) {
            if (isArray) {
                children.add(new SimpleNode(name, Arrays.asList(o)));
            } else {
                if (o instanceof Map) {
                    final Map<String, List> map = (Map<String, List>) o;
                    for (final Map.Entry<String, List> entry : map.entrySet()) {
                        children.add(new SimpleNode(entry.getKey(), entry.getValue()));
                    }
                } else if (o instanceof String) {
                    value = (String) o;
                } else if (o == null) {
                    throw new NullPointerException("Tried to create Node (" + name + ") with a null entry in the list.");
                } else {
