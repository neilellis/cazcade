...
public class ChangePasswordServlet extends AbstractBoardcastServlet {
    ...
    protected void doPost(@Nonnull final HttpServletRequest req, final HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            if (hash == null) {
                final ChangePasswordRequest response = dataStore.process(new ChangePasswordRequest(new SessionIdentifier(username)));
                ...
                if (!password.equals(passwordConfirm)) {
                    ...
                    final ChangePasswordRequest response = dataStore.process(new ChangePasswordRequest(new SessionIdentifier(username), password, hash));
                    if (response.response().error()) {
